	...
	<tr>
		...
		<td class="center">
			<?php echo ($column->allowNull
				? CHtml::checkBox($column->name . '[null]', is_null($row->getAttribute($column->name))) : ''); ?>
		</td>
		<td>
			<?php $this->widget('InputField', array(
	   'row' => $row, 'column' => $column, 'htmlOptions' => array(
		'onchange' => '$(this).val() ? $("#' . CHtml::$idPrefix . $column->name . '_null").attr("checked", "").change() : void(0);',
		'onkeyup' => '$(this).val() ? $("#' . CHtml::$idPrefix . $column->name . '_null").attr("checked", "").change() : void(0);',
		'tabIndex' => $i,
	)
  )); ?>
		</td>
