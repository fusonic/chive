	...
	<script type="text/javascript">
		...
		function save() {
			...
			$.post('<?php echo BASEURL; ?>/row/update',
			{
				...
				value:		JSON.stringify($('#<?php echo $id; ?>').val()),
			...
			function(response) {
				...
				if(responseObj.data.error) {
					$('#<?php echo $id; ?>').parent().parent().html(<?php echo json_encode($oldValue); ?>);
					AjaxResponse.handle(response);
					return false;
				}
	
				$('#<?php echo $id; ?>').parent().parent().html(responseObj.data.visibleValue);
