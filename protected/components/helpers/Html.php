...
class Html
{
	...
	public static function submitFormArea($container = true, $submit = true, $cancel = true)
	{
		if($submit)
		{
			$submitButton = '<a href="javascript:void(0)" onclick="$(\'#' . CHtml::ID_PREFIX . '\').submit()" class="icon button primary">'
					. Html::icon('save')
		...
		if($cancel)
		{
			$cancelButton = '<a href="javascript:void(0)" onclick="$(\'#' . CHtml::ID_PREFIX . '\').slideUp(500, function() { $(this).parents(\'tr\').remove(); })" class="icon button">'
					. Html::icon('delete')
